...
import { token2Address, token2Id, EXCHANGE, exchangeAbi } from "../constants";
...
function OrderMatch() {
  ...
  const matchOrders = async () => {
    ...
    console.table({"from": token2Address.get(orderOne.tokenFrom),
    ...
    const tx = await exchangeContract.executeOrder(
        token2Address[orderOne.tokenFrom],
        token2Address[orderTwo.tokenFrom],
        orderOne.orderFrm,
        orderTwo.orderFrm,
        token2Id[orderOne.tokenFrom],
        token2Id[orderTwo.tokenFrom],
        orderOne.quantity,
        orderTwo.quantity,
        data,
        {
