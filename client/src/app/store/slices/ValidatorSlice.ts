...
import { createAndLoadEncryptedSigner, depositIntoEncryptedSigner } from '../../oms/ValidatorService'
...
export const depositIntoSigner = createAsyncThunk(
  ...
  async (options: { primaryAccount: string; etherDeposit: string }, thunkAPI: any): Promise<void> => {
    await depositIntoEncryptedSigner(options.primaryAccount, options.etherDeposit)
  },
)
